...
public class NirvanaArchiveBot extends NirvanaBasicBot{
	...
	protected void go() {
		...
		// 4) нумераци€, сверху/снизу
		// 5) ботоправка,мала€ правка
		// ~4) удал€ть старые заголовки?
		// ~5) шапка
		// ~6) подвал
		...
		if (options.containsKey(key) && !options.get(key).isEmpty()) {
			...
			if(p1==Period.NONE) {
				log.error("¬ параметре \"формат заголовка в архиве\" не задан переменный параметр. «начение этого параметра не прин€то.");
				return;
			} else {
		...
		if (options.containsKey(key) && !options.get(key).isEmpty()) {
			...
			if(p2==Period.NONE) {
				log.error("¬ параметре \"формат подзаголовка в архиве\" не задан переменный параметр. «начение этого параметра не прин€то.");
				return;
			} else {
		...
		if(p1!=Period.NONE && p2!=Period.NONE && p1==p2) {
			log.error("ѕараметр \"формат заголовка в архиве\" и параметр \"формат подзаголовка в архиве\" имеют одинаковый период повторени€ "+p1.template());
			return;
		}
		...
		if (options.containsKey("помечать правки") && !options.get("помечать правки").isEmpty()) {
			...
			if(mark.equalsIgnoreCase("нет")) {
				...
				if(!mark.contains("бот") && mark.contains("мала€")) {
					bot = false;
				} else if(mark.contains("бот") && !mark.contains("мала€")) {
					minor = false;
				}				
			}
		}

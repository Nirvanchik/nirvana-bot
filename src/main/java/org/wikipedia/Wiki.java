...
/**
 *  Wiki.java has minor modifications (bug fixes and adaptations) made by Dmitry Trofimovich 15/11/2014
 */
...
public class Wiki implements Serializable
{
    ...
    private static Logger logger = Logger.getLogger("wiki");
    ...
    public String[] getPageText(String[] titles) throws IOException
    {
        ...
        log(Level.INFO, "getPagesText",
                "Successfully retrieved text of " + String.valueOf(titles2.length) +
                " pages: " + Arrays.toString(titles2));
        return ret;
    }
            ...
            String[] templatesToCheck) throws IOException
    {
        ...
        if (templatesToCheck != null && templatesToCheck.length > 0)
            request.append("&tltemplates=").append(constructTitleStringNoSplit(templatesToCheck));
        request.append("&titles=");
        String[] titleBunches;
        getUrl.append(request.toString());
        titleBunches = constructTitleStringForked(getUrl.length() + 100, titles, true);
        for (String temp : titleBunches)
        {
            ...
            do
            {
                ...
                // TODO: Migrate to POST, get rid of constructTitleStringForked()
                line = fetch(nextRequest.toString(), null, "getPagesTemplates");
     ...
     *  @since 0.31
     */
    public boolean[] hasTemplate(String[] titles, String template) throws IOException
    {
        return hasTemplates(titles, new String[] {template})[0];
    }
    ...
    public String[] whatLinksHereForked(String title, boolean redirects, boolean linksViaRedirects, int... ns) throws IOException
    {
        ...
        do
        {
            ...
            String line;
            if (blcontinue == null)
                line = fetch(url.toString(), null, "whatLinksHere");
            else
                line = fetch(url.toString() + "&blcontinue=" + blcontinue, null, "whatLinksHere");
     ...
     *  @since 0.29
     */
    protected String[] constructTitleStringForked(int lengthBaseUrl, String[] titles, boolean limit) throws IOException
    {
        if (titles.length == 0)
            return new String[0];
        ...
        {
            if (num < slowmax &&
                buffer.length() + titleStringToken.length() + titlesEnc[i].length() < urlMaxLength - lengthBaseUrl)
            {
    ...
    // TODO: Migrate to constructTitleString()
    /**
     *  Cuts up a list of titles into batches for prop=X&amp;titles=Y type queries.
     *  RaiseIgnores URL max allowed length.
     ...
     *  @return the titles ready for insertion into a URL
     *  @throws IOException if a network error occurs
     *  @throws IllegalArgumentException if title string violates max url length  
     *  @since 0.32
     */
    protected String constructTitleStringNoSplit(String[] titles) throws IOException
    {
        String [] parts = constructTitleStringForked(0, titles, true);
        if (parts.length > 1) {
        	throw new IllegalArgumentException(String.format(
        			"Too many titles. " +
        			"Cannot fit in one url without violating max url length %1$d and " +
        			"title max count of %2$d. Titles: ", urlMaxLength, slowmax));
        }
        return parts[0];
    }
